...
const getAvatarUrl = (path) => {
  if (!path || path === "avatar/default.png")
...
<template>
  ...
  >
    ...
    <ul v-if="isMainPage" class="hidden lg:flex list-none gap-x-[30px]">
      <li
        class="transition-colors duration-300 ease-in-out cursor-pointer group"
      >
        <router-link to="/home"
          ><button class="btn-glass group-hover:text-[#122f4f]">
            Beranda
          </button></router-link
        >
      </li>
    ...
    <div class="hidden lg:block w-48 relative">
      <div v-if="isLoggedIn" class="btn-glass text-white font-semibold">
        ...
        <transition name="fade">
          <div
            v-if="isProfileOpen"
            class="absolute top-full right-0 mt-2 w-full btn-glass p-2 bg-[#406691]/90 backdrop-blur-md border border-white/20 rounded-xl"
          >
      ...
      <div v-else class="flex gap-3 justify-end">
        <router-link to="/login">
          <button
            class="px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/30 transition-all text-white font-medium backdrop-blur-sm"
          >
            Masuk
          </button>
        </router-link>
        <router-link to="/register">
          <button
            class="px-5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium shadow-lg shadow-blue-900/20 transition-all"
          >
            Daftar
          </button>
        </router-link>
    ...
    >
      <span
        class="w-6 h-[2px] bg-white transition-all"
        :class="{ 'rotate-45 translate-y-[6px]': isMenuOpen }"
      ></span>
      <span
        class="w-6 h-[2px] bg-white"
    ...
    <transition name="fade">
      <div
        v-if="isMenuOpen"
        class="absolute top-[70px] left-0 w-full flex justify-center lg:hidden px-4"
      >
        <div
        ...
        >
          ...
          >
            ...
            <template v-else>
              <div
                class="w-[80px] h-[80px] rounded-full bg-white/10 flex items-center justify-center mb-2"
              >
              ...
              <div class="flex gap-3 mt-2">
                <router-link
                  to="/login"
                  class="px-4 py-1.5 bg-white/20 rounded-lg text-sm hover:bg-white/30"
                  >Masuk</router-link
                >
                <router-link
                  to="/register"
                  class="px-4 py-1.5 bg-blue-600 rounded-lg text-sm hover:bg-blue-700"
                  >Daftar</router-link
                >
              </div>
            </template>
          ...
          >
            ...
            <template v-else>
              <router-link
                @click="toggleMenu"
                v-for="item in navigationItemsShop"
                :key="item.name"
                :to="item.path"
                class="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-white/10 transition"
              >
