...
const getStatusClass = (status) => {
    switch (status) {
        case 'terbayar': return 'text-green-600 bg-green-50 px-2 py-1 rounded';
        case 'belum_dibayar': return 'text-yellow-600 bg-yellow-50 px-2 py-1 rounded';
        case 'dibatalkan': return 'text-red-600 bg-red-50 px-2 py-1 rounded';
        default: return 'text-gray-500 bg-gray-50 px-2 py-1 rounded';
    }
};
</script>

<template>
    <div class="bg-white rounded-xl shadow p-6 transition-all duration-300 border border-gray-100 hover:shadow-md">
        
        <div class="flex flex-col sm:flex-row justify-between gap-4">
            <div class="flex-1">
                <h2 class="text-lg font-semibold text-gray-900 line-clamp-2">{{ props.item.title }}</h2>
            ...
            <div class="flex-shrink-0 text-left sm:text-right">
                ...
                <span class="text-xs font-medium mt-1 inline-block" :class="getStatusClass(props.item.status)">
        ...
        <div class="mt-4 border-t border-gray-100 pt-4 flex justify-end">
            ...
            <div v-if="props.item.hasReviewed" class="flex items-center text-sm font-medium gap-1">
            ...
            >
                <Icon :icon="props.item.isRatingVisible ? 'mdi:chevron-up' : 'mdi:star-outline'" class="w-4 h-4"/>
                {{ props.item.isRatingVisible ? 'Tutup' : 'Beri Ulasan' }}
            </button>
        </div>

        <div v-if="props.item.isRatingVisible" class="mt-4 pt-4 border-t border-dashed border-gray-200 animate-fade-in">
            <div class="flex flex-col gap-3">
                <p class="text-sm font-medium text-gray-700">Bagaimana pengalaman Anda?</p>
                
                <div class="flex items-center gap-1">
                    <button v-for="star in 5" :key="star" @click="props.item.rating = star" class="focus:outline-none transition-transform active:scale-90">
                        <Icon 
                            :icon="star <= props.item.rating ? 'mdi:star' : 'mdi:star-outline'" 
