  ...
  try {
    ...
    const orders = orderRes.data.map((item) => {
      ...
      if (item.total_items > 1)
        title += ` (+ ${item.total_items - 1} barang lainnya)`;
...
const toggleRating = (clickedItem) => {
  ...
  if (clickedItem.status !== "terbayar") {
    // Status 'terbayar' kita set untuk paid/completed/delivered
  ...
  <div class="w-full min-h-screen bg-gray-100 p-4 md:p-8 font-sans">
    <header class="flex justify-between items-center mb-6">
      ...
      <button
        ...
        class="lg:hidden bg-white text-blue-600 p-2 border border-blue-600 rounded-full shadow-sm"
      >
        <Icon
          :icon="
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <main class="lg:col-span-2 flex flex-col gap-4">
        <div
          ...
          class="p-8 text-center bg-white rounded-xl shadow"
        >
