    ...
    <div class="flex flex-col items-center">
      ...
      <div class="flex items-center justify-center w-full max-w-xl mb-12">
        <div class="flex items-center">
          <div :class="getStepClass(1)"
            class="w-12 h-12 rounded-full flex items-center justify-center text-xl font-bold transition-all">
            <Icon v-if="currentStep > 1" icon="mdi:check" class="w-6 h-6" />
            <span v-else>1</span>
          </div>
        </div>
        <div class="flex-auto border-t-2 transition-all duration-500"
          :class="currentStep > 1 ? 'border-green-500' : 'border-gray-200'"></div>
        <div class="flex items-center">
        ...
        <div class="flex-auto border-t-2 transition-all duration-500"
          :class="currentStep > 2 ? 'border-green-500' : 'border-gray-200'"></div>
        <div class="flex items-center">
      ...
      <div class="w-full flex justify-center">
        ...
        <div v-if="currentStep === 1"
          class="bg-zinc-100 border-white shadow-md border-4 rounded-[2rem] p-8 w-full max-w-4xl">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="bg-[#eeeeee] border-4 border-white rounded-2xl overflow-hidden shadow">
              ...
              <div class="p-6 bg-transparent">
                ...
                <p class="text-2xl font-bold text-blue-600">Rp. 15.000,00 <span
                    class="text-lg font-normal text-gray-500">per 1 jam</span></p>
              </div>
            </div>
            ...
            <form @submit.prevent="handleStep1Submit">
              <div class="bg-[#eeeeee] border-4 border-white rounded-2xl space-y-6 col-span-1 p-6 mb-6">
                <div>
                  ...
                  <input v-model="formDataStep1.firstName" type="text" id="firstName"
                    class="mt-1 block w-full px-4 py-3 border border-gray-300 text-black rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Isi disini ...">
                </div>
                <div>
                  <label for="lastName" class="block text-sm font-medium text-gray-700">Nama Belakang</label>
                  <input v-model="formDataStep1.lastName" type="text" id="lastName"
                    class="mt-1 block w-full px-4 py-3 border border-gray-300 text-black rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Isi disini ...">
                </div>
                <div>
                  <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                  <input v-model="formDataStep1.email" type="email" id="email"
                    class="mt-1 block w-full px-4 py-3 border border-gray-300 text-black rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Isi disini ...">
                </div>
                <div>
                  <label for="phone" class="block text-sm font-medium text-gray-700">Nomor Telp</label>
                  <input v-model="formDataStep1.phone" type="tel" id="phone"
                    class="mt-1 block w-full px-4 py-3 border border-gray-300 text-black rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Isi disini ...">
                </div>
              </div>
              ...
              <div class="mt-8 flex flex-col space-y-3">
                ...
                <button type="button" @click="resetFlow"
                  class="w-full bg-red-500 text-white font-semibold py-3 px-6 rounded-lg hover:bg-red-600 transition duration-300">
                  Batalkan
                </button>
              </div>
            </form>
        ...
        <div v-if="currentStep === 2"
          ...
          <div class="flex flex-col items-center w-full">
            ...
            <div class="flex justify-between w-full px-2 sm:px-4">
              ...
              <div
                class="border-4 border-gray-300 rounded-lg w-2/3 h-32 sm:h-48 flex items-center justify-center text-gray-500 text-base sm:text-xl font-semibold text-center">
                Area Pemancingan
              </div>
          ...
          <div
            class="flex flex-col items-center space-y-4 sm:flex-row sm:space-y-0 sm:justify-center sm:space-x-6 mt-8">
            <div class="flex items-center space-x-2">
              <div class="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg bg-blue-600 text-white">
                <Icon icon="mdi:storefront" class="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
            ...
            <div class="flex items-center space-x-2">
              <div
                class="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg bg-white border border-gray-300">
                <Icon icon="mdi:storefront" class="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
              <span class="text-gray-700">Tersedia</span>
            </div>
            <div class="flex items-center space-x-2">
              <div class="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg bg-red-500 text-white">
                <Icon icon="mdi:storefront" class="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
          ...
          <div class="mt-8 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
            <button @click="goToStep(1)"
              class="bg-gray-300 text-gray-800 font-semibold py-3 px-8 rounded-lg hover:bg-gray-400 transition duration-300">
              Kembali
            </button>
            <button @click="handleStep2Submit"
              class="bg-blue-600 text-white font-semibold py-3 px-8 rounded-lg hover:bg-blue-700 transition duration-300">
              Selanjutnya
            </button>
          </div>
        </div>
              ...
              <div class="bg-zinc-100 border-white shadow-md border-4 rounded-[2rem] p-4 sm:p-8 w-full max-w-4xl">
                ...
                <div class="space-y-4">
                  <label class="flex items-center p-4 border rounded-lg cursor-pointer"
                    :class="{ 'border-blue-500 ring-2 ring-blue-500': paymentMethod === 'creditCard' }">
                  ...
                  <label class="flex items-center p-4 border rounded-lg cursor-pointer"
                    :class="{ 'border-blue-500 ring-2 ring-blue-500': paymentMethod === 'qris' }">
              ...
              <div class="bg-zinc-100 border-white shadow-md border-4 rounded-[2rem] p-4 sm:p-8 w-full max-w-4xl">
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  <input v-model="voucherCode" type="text"
                    class="flex-grow px-4 py-3 border border-gray-300 rounded-lg text-black focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Masukkan kode anda disini">
                  <button
                    class="bg-blue-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-blue-700 transition duration-300 w-full sm:w-auto">
                    Konfirmasi
                  </button>
                </div>
              </div>
            ...
            <div class="lg:col-span-1">
              <div class="bg-zinc-100 border-white shadow-md border-4 rounded-[2rem] p-4 sm:p-8 w-full max-w-4xl">
                ...
                <div class="mt-6">
                  ...
                  <textarea
                    class="mt-1 block w-full px-4 py-3 border border-gray-300 rounded-lg text-black focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    rows="3" placeholder="Catatan tentang order anda"></textarea>
                </div>
                ...
                <button @click="goToStep(2)"
                  class="w-full bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-lg hover:bg-gray-300 transition duration-300 mt-3">
                  Kembali
                </button>
