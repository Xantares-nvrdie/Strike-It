...
const reviews = ref([]);
const baseUrl = 'http://localhost:3000/uploads'; // Sesuaikan port backend
...
const fetchData = async () => {
    ...
    try {
        ...
        // Jika belum, kita pakai fallback ke loc.img utama
        const allImages = loc.images || [];
        
        // Cari gambar tipe 'main' (atau pakai cover default)
        const mainImgObj = allImages.find(i => i.img_type === 'main') || { img_path: loc.img };
        ...
        // 2. Ambil Review Lokasi
        const revRes = await api.getLocationReviews(id);
        reviews.value = revRes.data.map(r => ({
            id: r.id,
            username: r.username,
            // Handle avatar user
            avatarUrl: r.avatarUrl ? getFullUrl(r.avatarUrl) : `https://ui-avatars.com/api/?name=${r.username}&background=random`,
...
function handleTouchEnd(e) {
    ...
    if (Math.abs(diffX) > 50) {
        ...
        if (diffX < 0) { // Swipe Kiri -> Next Tab
            activeTab.value = tabs[(current + 1) % tabs.length];
        } else { // Swipe Kanan -> Prev Tab
...
<template>
    <div v-if="!locationData" class="flex flex-col justify-center items-center h-screen text-gray-500 gap-4">
        <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-blue-600"></div>
        <p>Memuat informasi lokasi...</p>
    </div>

    <main v-else class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 w-full bg-zinc-100 rounded-3xl shadow-xl p-4 md:p-8 lg:p-12 flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-8">
                    ...
                    <div class="bg-zinc-100 border-white border-4 rounded-2xl shadow-xl p-6 md:p-8">
                        ...
                        <div class="flex flex-col gap-6">
                            ...
                            <div v-if="reviews.length === 0" class="text-center text-gray-500 italic py-4">
            ...
            <div class="block md:hidden space-y-4">
                <div class="relative w-full">
                    <img :src="locationData.mainImage" alt="Main Location"
                    ...
                    <div class="absolute inset-0 flex justify-between items-center px-2 pointer-events-none">
                        <button class="bg-white/70 rounded-full p-1 text-black shadow-md pointer-events-auto hover:bg-white">
                ...
                <div class="flex justify-center gap-2 py-2">
                    <button @click="activeTab = 'about'" 
                        :class="activeTab === 'about' ? 'bg-blue-600 w-8' : 'bg-gray-300 w-3'"
                        class="h-3 rounded-full transition-all duration-300"></button>
                    <button @click="activeTab = 'location'" 
                        :class="activeTab === 'location' ? 'bg-blue-600 w-8' : 'bg-gray-300 w-3'"
                        class="h-3 rounded-full transition-all duration-300"></button>
                    <button @click="activeTab = 'review'" 
                        :class="activeTab === 'review' ? 'bg-blue-600 w-8' : 'bg-gray-300 w-3'"
                        class="h-3 rounded-full transition-all duration-300"></button>
                </div>

                <div ref="tabContentRef" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
                    <div class="bg-[#eeeeee] border-4 border-white rounded-2xl shadow-xl p-6 min-h-[300px]">
                        ...
                        <div v-if="activeTab === 'location'" class="animate-fade-in">
                            <h3 class="text-2xl font-medium text-center text-gray-900 mb-4">Peta Lokasi</h3>
                        ...
                        <div v-if="activeTab === 'review'" class="animate-fade-in">
                            ...
                            <div class="flex flex-col gap-4">
                                <ReviewCard v-for="review in reviews" :key="review.id" :review="review" />
                                <p v-if="reviews.length === 0" class="text-center text-gray-500 italic">Belum ada ulasan.</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="hidden lg:block lg:col-span-1">
            <div class="sticky top-24 flex flex-col gap-8">
                <BookingCard :price="Number(locationData.price)" :locationId="locationData.id" />
        ...
        <div v-if="isBookingModalOpen" @click="isBookingModalOpen = false"
            ...
            <div @click.stop class="relative bg-white rounded-2xl p-4 pt-10 shadow-2xl w-full max-w-md animate-slide-up">
