...
<template>
  <div id="penilaian" class="relative w-full overflow-hidden antialiased bg-gray-50 font-outfit">
    <div class="absolute top-0 left-0 z-10 w-full h-48 pointer-events-none bg-gradient-to-b from-gray-50 via-gray-50 to-transparent"></div>
    ...
    <div class="relative w-full max-w-7xl mx-auto h-[50rem] overflow-hidden">
      <div v-if="reviews.length === 0" class="flex justify-center items-center h-full text-gray-400">
          Belum ada ulasan lokasi saat ini.
      </div>

      <div v-else class="absolute inset-0 grid grid-cols-1 gap-6 px-6 md:grid-cols-3">
    ...
    <div class="absolute bottom-0 left-0 z-10 w-full h-48 pointer-events-none bg-gradient-to-t from-blue-700 to-transparent"></div>
  </div>
</template>
