...
function goToPage(n) { currentPage.value = n; }
</script>

<template>
    <section class="w-full p-3 sm:p-5 bg-linear-131 mt-5 from-white/20 to-gray-50/20 rounded-[37px] shadow-lg outline outline-[2.5px] outline-white backdrop-blur-sm flex flex-col gap-4">
        ...
        <ul v-if="displayMode === 'grid'" class="w-full grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <li v-for="product in gridProducts" :key="product.id" class="flex flex-col">
        ...
        <div v-else-if="displayMode === 'row'" class="w-full">
            <ul class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <li v-for="product in rowProducts" :key="product.id" class="flex flex-col">
                    <ProductCard :product="product" class="w-full h-full" />
                </li>
            </ul>
        </div>

        <div v-if="displayMode === 'grid' && totalPages > 1" class="flex items-center justify-center w-full mt-6 gap-2">
            <button 
                @click="prevPage" :disabled="currentPage === 1"
                class="p-2 bg-white rounded-lg shadow hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                <Icon icon="heroicons:arrow-left-20-solid" class="w-5 h-5 text-gray-700" />
            </button>
            ...
            <button
                ...
                :class="n === currentPage ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'"
            >
                {{ n }}
            </button>
            ...
            <button 
                ...
                class="p-2 bg-white rounded-lg shadow hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                <Icon icon="heroicons:arrow-right-20-solid" class="w-5 h-5 text-gray-700" />
            </button>
        </div>
