...
const addToCart = async () => {
  ...
  if (!canBuy.value) {
    toast.warning(
      "Produk sewa hanya untuk showcase, tidak untuk checkout online.",
    );
    return;
  }
...
<template>
  <section
    v-if="product"
    class="w-full min-h-screen flex flex-col justify-start items-start bg-zinc-100 pt-[100px] px-4 md:px-10"
  >
    <div
      class="w-full p-5 bg-zinc-100 flex flex-col lg:flex-row justify-center items-start gap-10 rounded-lg"
    >
      <div
        class="flex-1 w-full bg-zinc-100 shadow-[0px_10px_1px_rgba(221,_221,_221,_1),_0_10px_20px_rgba(204,_204,_204,_1)] p-5 rounded-lg flex flex-col justify-center"
      >
        <img
          :src="mainImage"
          :alt="product.name"
          class="max-w-[513px] max-h-[497px] w-full object-cover mx-auto rounded-lg aspect-square"
        />
        <div
          v-if="thumbnailImages.length > 1"
          class="flex flex-row items-center mt-10 gap-4 overflow-x-auto whitespace-nowrap px-4 py-2 bg-zinc-100"
        >
          <img
      ...
      <div class="flex-1 w-full bg-zinc-200 p-5 rounded-lg text-black">
        ...
        <div class="mt-4 flex flex-col gap-2">
          ...
          >
            <span
              class="text-xs font-bold bg-green-600 text-white px-2 py-1 rounded uppercase tracking-wider"
              >BELI</span
            >
            <span class="text-3xl font-bold text-green-700"
              >Rp.
              {{ Number(product.price_sale).toLocaleString("id-ID") }}</span
            >
          </div>
          <div
            v-if="canRent"
            class="flex items-center gap-3 p-2 bg-blue-50 rounded-lg border border-blue-100 opacity-75"
          >
            <span
              class="text-[10px] font-bold bg-blue-600 text-white px-2 py-0.5 rounded uppercase tracking-wider"
              >SEWA</span
            >
            <span class="text-lg font-medium text-blue-700"
              >Rp. {{ Number(product.price_rent).toLocaleString("id-ID") }}
        ...
        <div
          v-if="canRent && !canBuy"
          class="mt-5 text-blue-700 font-medium bg-blue-100 p-3 rounded-lg text-sm"
        >
          Produk hanya dapat disewa di lokasi / tempat. Tidak tersedia untuk
          checkout online.
        </div>
        ...
        <div class="mt-4" v-if="canBuy">
          <p
            ...
            class="inline-flex items-center gap-2 text-green-700 font-medium bg-green-100 px-3 py-1 rounded-full text-sm"
          >
            <Icon icon="tabler:checkbox" class="w-5 h-5" /> Stok Tersedia:
            {{ product.stock }}
          </p>
          <p
            v-else
            class="inline-flex items-center gap-2 text-red-600 font-medium bg-red-100 px-3 py-1 rounded-full text-sm"
          >
            <Icon icon="healthicons:no-outline-24px" class="w-5 h-5" /> Stok
            Habis
          </p>
        </div>
        ...
        >
          <Icon
            :icon="isAddedToCart ? 'mdi:check' : 'mdi:cart-plus'"
            class="w-5 h-5"
          />
  ...
  <div
    v-else
    class="h-screen flex flex-col justify-center items-center text-gray-500 gap-4"
  >
    <div
