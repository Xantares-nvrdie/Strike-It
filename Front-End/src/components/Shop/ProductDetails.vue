...
const fetchDetail = async () => {
    ...
    try {
        ...
        if (allImages.length > 0) {
            ...
            if (mainObj) mainImgPath = mainObj.img_path;
            else mainImgPath = allImages[0].img_path;
        }
        
        let galleryUrls = [];
        if (allImages.length > 0) {
            galleryUrls = allImages.map(i => getFullUrl(i.img_path));
        } else {
            galleryUrls = [getFullUrl(mainImgPath)];
        }

        product.value = p;
        mainImage.value = getFullUrl(mainImgPath);
        thumbnailImages.value = galleryUrls;
...
const addToCart = async () => {
    ...
    // Validasi: Hanya barang yang dijual yang bisa masuk keranjang belanja
    if (!canBuy.value) {
        toast.warning("Produk sewa tidak masuk keranjang belanja, silakan langsung 'Sewa Sekarang'.");
        return;
    }
...
const handleTransaction = async (type) => {
    ...
    try {
        await api.addToCart({ 
            id_product: product.value.id, 
            quantity: quantity.value,
            transaction_type: type // 'sewa' atau 'beli'
        });
            ...
            <div class="flex-1 w-full bg-zinc-200 p-5 rounded-lg text-black">
                ...
                <div class="mt-4 flex flex-col gap-2">
                    ...
                    <div v-if="canRent" class="flex items-center gap-3 p-2 bg-blue-50 rounded-lg border border-blue-100 opacity-75">
                        ...
                        <span class="text-lg font-medium text-blue-700">Rp. {{ Number(product.price_rent).toLocaleString('id-ID') }} <span class="text-sm font-normal text-gray-500">/hari</span></span>
                    </div>
                    
                    <div v-if="!canBuy && !canRent" class="p-3 bg-red-50 rounded-lg border border-red-100 text-red-600 font-medium">
                        <Icon icon="mdi:alert-circle-outline" class="inline w-5 h-5 mr-1"/>
