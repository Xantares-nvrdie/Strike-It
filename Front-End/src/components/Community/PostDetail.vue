...
const loadData = async () => {
    ...
    try {
        ...
        comments.value = commentsRes.data.map(c => ({
            ...
            author: {
                name: c.author_name,
                avatar: c.author_avatar ? (c.author_avatar.startsWith('http') ? c.author_avatar : `${baseUrl}/${c.author_avatar}`) : `https://ui-avatars.com/api/?name=${c.author_name}`
            },
...
const submitComment = async () => {
    ...
    try {
        ...
        loadData(); // Refresh data komentar
    } catch (error) {
